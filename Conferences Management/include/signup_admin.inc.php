...
if(isset($_POST["signup_submit"])) {
    ...
    if(empty($uid) || empty($fname) || empty($lname) || empty($pwd) || empty($pwd_repeat)) {
        header("Location: ../AddNewAdmin.php?error=emptyfields&uid=".$uid."&fname=".$fname."&lname=".$lname);
        exit();
    }elseif(!preg_match("/^[a-zA-Z0-9]+$/", $uid)) {
        header("Location: ../AddNewAdmin.php?error=invaliduid&fname=".$fname."&lname=".$lname);
        exit();
    }elseif($pwd !== $pwd_repeat) {
        header("Location: ../AddNewAdmin.php?error=passworderror&uid=".$uid."&fname=".$fname."&lname=".$lname);
        exit();
    }else {
        $sql = "SELECT account_uid FROM account WHERE account_uid=?;";
        $statement = mysqli_stmt_init($conn);
        if(!mysqli_stmt_prepare($statement, $sql)) {
            header("Location: ../AddNewAdmin.php?error=sqlerror1");
            exit();
        }else {
            ...
            if($result > 0) {
                ...
                header("Location: ../AddNewAdmin.php?error=usertaken&uid=".$uid."&fname=".$fname."&lname=".$lname);
                exit();
            }else {
                ...
                if(!mysqli_stmt_prepare($statement, $sql)) {
                    header("Location: ../AddNewAdmin.php?error=sqlerror2");
                    exit();
                }else {
                    //hash the password
                    ...
                    mysqli_stmt_execute($statement);
                    header("Location: ../AddNewAdmin.php?signup=success");
                    exit();
                }
            }
        }
